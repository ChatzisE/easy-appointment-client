<template>
  <v-data-table :headers="headers" :items="appointementsList" sort-by="date" class="elevation-1">
      ...
      <v-toolbar flat color="white">
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">New Appointment</v-btn>
          </template>
          <v-card>
            <v-card-title>
                ...
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="4">
                    ...
                    >
                      ...
                      <v-date-picker v-model="editedItem.appointment_date" @input="dateMenu = false"></v-date-picker>
                    </v-menu>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-menu
    ...
    <template v-slot:item.appointment_datetime="{ item }">{{ item.appointment_datetime | moment }}</template>
  </v-data-table>
</template>
<script>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Date", value: "appointment_datetime" },
  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      confirm("Are you sure you want to del       debuggerete this appointment?") &&
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        //--todo emit and save new appointment  
        this.appointementsList.push(this.editedItem);
      }
      this.close();
    }
  }
